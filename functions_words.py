...
def findSpaces(line, thres_space):
    ...
    for i in range(0, line.shape[1]):
        if (not isSpace):
            if (hist[0][i]): # get the first starting x-coordinates and start count at 1
                isSpace = True
                count = 1
                x = i
        else:
