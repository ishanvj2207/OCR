...
def get_words(raw_image):
   ...
   # M is transformation matrix
    M = getTransformationMatrix(img_for_det) 
    ...
    for i in ycoords:
        cv2.line(img_with_lines,(0,np.float32(i)),(img_with_lines.shape[1],np.float32(i)),255,2)
    ...
    for i in range ( 0, len(ycoords)-1 ):
        ...
        #cv2.imwrite('line_'+str(i)+'.png', line)
        
        #finding the x-coordinates of the spaces
        xcoords = findSpaces(line, threshold_space)
        
        for x in xcoords:
            cv2.line(line, (np.float32(x),0), (np.float32(x),line.shape[0]), 255, 1)
        #cv2.imwrite('img/'+str(i)+'.png', line)
        
        count = 0
        
        for j in range (0, len(xcoords)-1 ): 
            line = img_for_ext[range(int(ycoords[i]),int(ycoords[i+1]))]
            
            word = line[:, int(xcoords[j]): int(xcoords[j+1])]
            all_words.append(word)
            #cv2.imwrite('zword_'+str(number_of_words)+'.png', word)
